...
for (vp in 1:n_vp_total) {
  ...
  for (face in faces_new) {
    ...
    # Source Judgment für False Alarms (für MPT-Modellierung)
    if (is_false_alarm == 1) {
      # Guessing - zufällige Source-Zuordnung
      source_judgment <- sample(c("trustworthy", "untrustworthy", "dont_know"),
                               1, prob = c(0.40, 0.40, 0.20))
      source_judgment_rt <- round(rtruncnorm(1, a = 400, b = 6000,
                                             mean = rt_source_mean * 1.1,
                                             sd = rt_source_sd))
    } else {
      source_judgment <- NA
      source_judgment_rt <- NA
...
# Note: Aggregated data doesn't have face_id, so we use only vp_id random effect
# The full trial-level GLMM with face_id is in analysis_pipeline.Rmd
model_test <- lmer(
