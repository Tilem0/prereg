...
data_person <- data_person %>%
  ...
  mutate(
    longest_string = {
      vals <- c(lzo_item_01, lzo_item_02, lzo_item_03, lzo_item_04, lzo_item_05,
                lzo_item_06, lzo_item_07, lzo_item_08, lzo_item_09, lzo_item_10)
      max(rle(vals)$lengths)
    },
...
```{r lmm-model}
# Prepare trial-level data for GLMM (binary DV: correct_source)
# Filter to old items that were hits
data_trial_hits <- data_cleaned %>%
  filter(item_type == "old", is_hit == TRUE) %>%
  left_join(data_person %>% select(vp_id, lzo_score, condition_ecology), by = "vp_id") %>%
  mutate(
    # Compute LZO centered at grand mean
    lzo_centered = lzo_score - mean(data_person$lzo_score, na.rm = TRUE),
    # Factor coding for emmeans compatibility
    valence_encoded = factor(valence_encoded, levels = c("trustworthy", "untrustworthy")),
    condition_ecology = factor(condition_ecology, levels = c("realistic", "inverted")),
    # Binary DV: correct source (0/1)
    correct_source = as.numeric(is_correct_source)
  )

# Set deviation contrasts (-0.5/0.5) so coefficients match preregistered contrast coding
contrasts(data_trial_hits$valence_encoded) <- contr.sum(2) / 2
contrasts(data_trial_hits$condition_ecology) <- contr.sum(2) / 2

# Maximale Struktur (gemäß Präregistrierung): trial-level GLMM with binomial family
model_full <- glmer(
  correct_source ~ condition_ecology * valence_encoded * lzo_centered +
         (1 + valence_encoded | vp_id) +
         (1 | face_id),
  data = data_trial_hits,
...
if(isSingular(model_full)) {
  ...
  if(isSingular(model_reduced)) {
    cat("Still singular. Trying without random slope...\n")
    model_noslope <- glmer(
      correct_source ~ condition_ecology * valence_encoded * lzo_centered +
             (1 | vp_id) +
             (1 | face_id),
    ...
    if(isSingular(model_noslope)) {
      cat("Still singular. Using simplest structure (vp_id only)...\n")
      model_final <- glmer(
        correct_source ~ condition_ecology * valence_encoded * lzo_centered +
               (1 | vp_id),
        data = data_trial_hits,
...
```{r lmm-fixed-effects}
# ANOVA Tabelle (Type III) - use car::Anova for GLMM
anova_table <- car::Anova(model_final, type = "III")
print(anova_table)
...
kable(anova_df, caption = "ANOVA Type III Results") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  row_spec(which(anova_df$`Pr(>Chisq)` < 0.05), bold = TRUE, color = "red")
```
...
```{r hypothesis-h1-h2}
# Estimated Marginal Means (type = "response" for back-transformed probabilities)
...
```{r hypothesis-h3}
# Dreifach-Interaktion aus ANOVA (car::Anova returns Pr(>Chisq) for GLMMs)
# Find the three-way interaction row by matching the name
three_way_row <- grep("condition_ecology:valence_encoded:lzo_centered", rownames(anova_table), value = TRUE)
if (length(three_way_row) > 0) {
  three_way_p <- anova_table[three_way_row, "Pr(>Chisq)"]
} else {
  three_way_p <- NA
}
...
if(!is.na(three_way_p) && three_way_p < 0.05) {
  # Simple Slopes für LZO
  emtrends_lzo <- emtrends(model_final, 
                           ~ condition_ecology | valence_encoded, 
                           var = "lzo_centered")
  ...
  pred_data <- expand.grid(
    condition_ecology = factor(c("realistic", "inverted"), levels = c("realistic", "inverted")),
    valence_encoded = factor(c("trustworthy", "untrustworthy"), levels = c("trustworthy", "untrustworthy")),
    lzo_centered = lzo_values
  )
  
  # Set same contrasts as in the model data
  contrasts(pred_data$condition_ecology) <- contr.sum(2) / 2
  contrasts(pred_data$valence_encoded) <- contr.sum(2) / 2
  
  # Predict on response scale (probabilities)
  pred_data$prob_pred <- predict(model_final, newdata = pred_data, type = "response", re.form = NA)
  ...
  p_moderation <- ggplot(pred_data, 
                        aes(x = valence_encoded, y = prob_pred, 
                            color = condition_ecology, group = condition_ecology)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
...
```{r effect-sizes}
# R² (Marginal & Conditional) - works for GLMMs too
...
# Odds Ratios for Fixed Effects (GLMM)
cat("=== Odds Ratios ===\n")
or_estimates <- exp(fixef(model_final))
print(round(or_estimates, 3))

cat("\n95% Confidence Intervals (Wald):\n")
or_ci <- exp(confint(model_final, method = "Wald", parm = "beta_"))
print(round(or_ci, 3))

# Create a summary table
or_table <- data.frame(
  Effect = names(fixef(model_final)),
...
kable(or_table, caption = "Odds Ratios with 95% CI") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# Modell mit Age und Gender als Kovariaten (GLMM on trial-level data)
model_covariates <- glmer(
  correct_source ~ condition_ecology * valence_encoded * lzo_centered +
         age + gender +
         (1 | vp_id) +
         (1 | face_id),
...
cat("\nOriginal Model:\n")
print(car::Anova(model_final, type = "III"))

cat("\nWith Covariates:\n")
print(car::Anova(model_covariates, type = "III"))
```
...
# d' is aggregated per VP × Valence, so no face_id random effect
# Prepare factor variables with deviation contrasts for data_analysis
data_analysis <- data_analysis %>%
  mutate(
    valence_encoded = factor(valence_encoded, levels = c("trustworthy", "untrustworthy")),
    condition_ecology = factor(condition_ecology, levels = c("realistic", "inverted"))
  )
contrasts(data_analysis$valence_encoded) <- contr.sum(2) / 2
contrasts(data_analysis$condition_ecology) <- contr.sum(2) / 2

model_dprime <- lmer(
  d_prime ~ condition_ecology * valence_encoded * lzo_centered +
            (1 | vp_id),
  data = data_analysis,
...
# Modell mit Gender-Interaktion (explorativ!) - use GLMM on trial-level data
model_gender <- glmer(
  correct_source ~ condition_ecology * valence_encoded * gender +
         (1 | vp_id) +
         (1 | face_id),
  data = data_trial_hits,
  family = binomial(link = "logit"),
...
print(car::Anova(model_gender, type = "III"))
```
...
rt_analysis <- data_cleaned %>%
  ...
  mutate(is_correct_source = factor(is_correct_source, 
                                     levels = c(FALSE, TRUE),
...
results_summary <- data.frame(
  ...
  Result = c(
    ...
    ifelse(exists("three_way_p"),
           paste("Chisq =", round(anova_table["condition_ecology:valence_encoded:lzo_centered", "Chisq"], 2),
